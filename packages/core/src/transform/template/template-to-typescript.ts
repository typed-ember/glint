...
export type TemplateToTypescriptOptions = {
  ...
  ) => Pick<TemplateToTypescriptOptions, 'globals' | 'preamble'> & { template: string };
...
export function templateToTypescript(
  ...
  return mapTemplateContents(
    originalTemplate,
    { embeddingSyntax, postprocessAst: args.postprocessAst },
    (ast, mapper) => {
      ...
      Object.keys(args.mapTemplateContent || {}).forEach((key) => {
        ...
        if (args.mapTemplateContent![k]) {
          emitters[k] = function (...emitterArgs: any) {
            return args.mapTemplateContent![k](
              originalEmitters[k],
              originalEmitters,
              mapper,
              ...emitterArgs
