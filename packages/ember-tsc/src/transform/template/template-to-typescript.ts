...
export function templateToTypescript(
  ...
  // Cache hit: same template string → same result. This assumes that other options
  // (typesModule, globals, specialForms, etc.) don't change between calls for the
  // same file, which holds because they come from the stable environment config.
  if (templateContext?.result && templateContext.template === originalTemplate) {
  ...
  let result = mapTemplateContents(
    originalTemplate,
    { embeddingSyntax, templateContext },
    (ast, mapper) => {
