import { Globals, resolve, applySplattributes, emitComponent, NamedArgsMarker, } from '@glint/environment-ember-loose/-private/dsl';
...
// <LinkTo>
{
    ...
    expectTypeOf(component.element).toEqualTypeOf();
...
{
    const component = emitComponent(LinkTo({ route: 'index', query: { a: 123 }, ...NamedArgsMarker }));
    expectTypeOf(component.blockParams.default).toEqualTypeOf();
}
{
    const component = emitComponent(LinkTo({ route: 'index', models: [123, 'abc'], ...NamedArgsMarker }));
    expectTypeOf(component.blockParams.default).toEqualTypeOf();
}
{
    const component = emitComponent(LinkTo({ route: 'index', 'current-when': 'index', ...NamedArgsMarker }));
    expectTypeOf(component.blockParams.default).toEqualTypeOf();
}
{
    const component = emitComponent(LinkTo({ route: 'index', 'current-when': true, ...NamedArgsMarker }));
    expectTypeOf(component.blockParams.default).toEqualTypeOf();
}
// Requires at least one of `@route`, `@model`, `@models` or `@query`
{
    const component = emitComponent(LinkTo(
...
//# sourceMappingURL=link-to.test.js.map