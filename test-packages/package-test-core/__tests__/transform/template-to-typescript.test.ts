    ...
    describe('{{fn}} (bind-positional)', () => {
      test('in top-level position', () => {
        ...
        expect(templateBody(template, { specialForms, globals: ['testFn'] })).toMatchInlineSnapshot(
          `"__glintDSL__.emitContent(__glintDSL__.resolve(__glintDSL__.Globals.testFn)(foo, 123));"`,
        );
      });
      ...
      test('in a subexpression inside another invocation', () => {
        ...
        expect(templateBody(template, { specialForms, globals: ['testFn'] })).toMatchInlineSnapshot(
          `"__glintDSL__.emitContent(__glintDSL__.resolve(someHelper)(__glintDSL__.resolve(__glintDSL__.Globals.testFn)(foo, 123)));"`,
        );
      });
      ...
      test('as an imported (non-global) binding', () => {
        ...
        expect(templateBody(template, { specialForms, globals: [] })).toMatchInlineSnapshot(
          `"(__glintDSL__.noop(testFn), __glintDSL__.emitContent(__glintDSL__.resolve(testFn)(foo)));"`,
        );
      });

      test('inside a bind-invokable keyword', () => {
        let template = '{{testComponent Foo onChange=(testFn foo 123)}}';
        let specialForms = { testFn: 'bind-positional', testComponent: 'bind-invokable' } as const;
    ...
    describe('paths', () => {
      describe('path types', () => {
        ...
        test('chained path with a $ as the first character of a property', () => {
          ...
          expect(templateBody(template, { globals: [] })).toMatchInlineSnapshot(
            `"__glintDSL__.emitContent(__glintDSL__.resolveOrReturn(obj?.$foo?.baz)());"`,
          );
        });
