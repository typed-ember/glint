...
// Pre-bound named arg
const PreBoundNamedTest = <template>
  {{#let (modifier Render3DModelModifier origin=(hash x=0 y=0)) as |boundRender|}}
...
const PrebindingTest = <template>
  ...
  <div {{(modifier MyriadPositionals "one" true)}}></div>

  {{! @glint-expect-error: extra arg }}
  <div {{(modifier MyriadPositionals "one" true 3) "four"}}></div>
</template>;
